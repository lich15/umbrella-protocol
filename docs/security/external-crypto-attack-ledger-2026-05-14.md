...
| MLS | RFC 9420 `https://www.rfc-editor.org/rfc/rfc9420` | 2026-05-14 | downgrade и group-state inconsistency | `umbrella-mls` | закрыто тестом | focused Rust package tests plus SFrame fuzz smoke evidence under `target/fuzz-overnight/20260514-185713` |
| SFrame | RFC 9605 `https://www.rfc-editor.org/rfc/rfc9605` | 2026-05-14 | tampered frame/header, nonce/key misuse | `umbrella-calls`, `umbrella-vectors` | закрыто тестом | focused Rust package tests plus SFrame fuzz smoke evidence under `target/fuzz-overnight/20260514-185713` |
| Устройства | WebAuthn `https://www.w3.org/TR/webauthn-3/` | 2026-05-14 | rollback счётчика и mismatch ключа | `umbrella-platform-verifier` | закрыто тестом | локальные WebAuthn тесты проходят; Apple App Attest и Android Play Integrity остаются закрыты отказом, пока не подключены внешние корни доверия и серверно-мобильная связка. |
| Устройства | Apple App Attest `https://developer.apple.com/documentation/devicecheck/validating-apps-that-connect-to-your-server` | 2026-05-14 | fake attestation без Apple trust material | `umbrella-platform-verifier` | закрыто отказом | локальные WebAuthn тесты проходят; Apple App Attest и Android Play Integrity остаются закрыты отказом, пока не подключены внешние корни доверия и серверно-мобильная связка. |
| Устройства | Android Play Integrity `https://developer.android.com/google/play/integrity` | 2026-05-14 | fake verdict без Google trust material | `umbrella-platform-verifier` | закрыто отказом | локальные WebAuthn тесты проходят; Apple App Attest и Android Play Integrity остаются закрыты отказом, пока не подключены внешние корни доверия и серверно-мобильная связка. |
