...
| Модели Tamarin | `bash scripts/verify-tamarin-models.sh` | 0 | Прошло (снимок 2026-05-13: 9 моделей Tamarin). После v1.1.0 количество Tamarin моделей выросло до 14 (добавлены: `aggressive_dh_pcs.spthy`, `spqr_deniability.spthy`, `discovery.spthy`, `sealed_servers_threshold_3of5.spthy`, `sealed_servers_threshold_universal.spthy` в раунде 7 + Pass 5 + Max Ratchet v3 Task 5); плюс 4 модели ProVerif (`backup_wrap_v2.pv`, `oprf_ristretto255.pv`, `sealed_sender_v1.pv`, `sealed_sender_v2.pv`); **Всего 18 формальных моделей в HEAD `938d1a89` (v3.0.0)**. |
