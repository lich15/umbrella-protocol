...
### 1.1 Scope-limit disclosure (M-FINAL-1 from independent review 2026-05-18)

**Acceptance gate 5 (R20 lldb scan = 0 identity_sk hits in 2.2 GB process
memory) applies to the round-6 distributed bootstrap path only**
(`distributed_identity_client::bootstrap_account` invoked from
`OnboardingHandle`). A separate entry point still exists in the codebase:

- **File:** `crates/umbrella-client/src/core.rs:420-422`
  (`ClientCore::new_with_hw_callback`)
- **Behavior:** synthesizes a 32-byte ephemeral signing seed via
  `IdentitySeed::generate(&mut OsRng, ...)` and derives a throwaway
  `IdentityKey` for backwards compatibility with downstream readers of
  `core.identity`.
- **Status:** the seed is `Box<[u8; 32]>` heap-resident, zeroize-on-drop,
  no stack spill (round-5 closure preserved). Window of existence:
  microseconds. Production HW signing path uses
  `core.hw_callback` and **never** uses this ephemeral.
- **Why this is not a vulnerability:** the ephemeral is dropped before
  control returns to the caller; it is not persisted, not exported,
  and not used for any production signing. Any lldb scan of the
  *running* process post-bootstrap will not find it (it is gone).
- **Why this is still disclosed:** the R20 claim says "0 identity_sk
  hits anywhere in this process at any point during bootstrap". For
  the round-6 distributed path that is verified end-to-end. For
  `new_with_hw_callback` there is a microseconds-wide window where
  a 32-byte signing key exists on the heap before being dropped.
  Honest disclosure is preferable to overgeneralizing.
- **Remediation track:** refactor `core.identity` to
  `Option<Arc<IdentityKey>>` or to a public-only verifying-key variant
  so this synthesis can be eliminated entirely. Tracked as v1.2.x
  M-FINAL-1.

This entry point is exercised by integration tests
(`crates/umbrella-client/tests/r5_hw_callback_wiring.rs`) and is the
existing HW-keystore-backed bootstrap from round-5 closure
(F-PHD-DC-R10-1). It is NOT removed in this round — only annotated
with the scope-limit comment in `core.rs:407-419` and tracked for
v1.2.x rewiring.
